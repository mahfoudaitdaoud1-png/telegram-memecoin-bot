...
# --- PATCH: robust loader + admin commands ---
def _merge_handles(*sources: List[str]) -> Set[str]:
    out: Set[str] = set()
    for src in sources:
        if not src:
            continue
...
def _load_env_following() -> Set[str]:
    # Optional comma/space separated list of handles via env
    inline = os.getenv("MY_FOLLOWING_INLINE", "")  # e.g. "cobie, foobar, 0xfoobar"
    return _merge_handles(inline)

def reload_following() -> int:
    """Recompute MY_HANDLES from file + env at runtime and persist file if empty."""
    global MY_HANDLES
    file_handles = load_my_following() or set()
    env_handles  = _load_env_following()
    merged       = (file_handles or set()) | env_handles
    # If file was missing and env provided, persist to file so it survives next cold start.
    if not file_handles and merged:
        try:
            p = pathlib.Path(MY_FOLLOWING_TXT)
            p.parent.mkdir(parents=True, exist_ok=True)
            p.write_text("\n".join(sorted(merged)), encoding="utf-8")
            log.info(f"[followed-by] wrote {len(merged)} handles to {p}")
        except Exception as e:
            log.warning(f"[followed-by] persist failed: {e}")
    MY_HANDLES = merged
    log.info(f"[followed-by] MY_HANDLES loaded: {len(MY_HANDLES)}")
    return len(MY_HANDLES)

# Optional: tidier ellipsis and deterministic cap
...
# Admin commands
async def cmd_reload_following(u: Update, c: ContextTypes.DEFAULT_TYPE):
    n = reload_following()
    await u.message.reply_text(f"[followed-by] reloaded {n} handles")

async def cmd_setfollowing(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """
    Usage:
      - Reply to a text file/message containing handles (one per line) with: /setfollowing
      - Or: /setfollowing handle1 handle2 handle3
    Persists to MY_FOLLOWING_TXT and reloads.
    """
    handles: Set[str] = set()
    parts = (u.message.text or "").split(maxsplit=1)
    if len(parts) == 2:
        handles |= _merge_handles(parts[1])
    # If message is a reply, try to use replied text as source
    if u.message.reply_to_message and (u.message.reply_to_message.text or u.message.reply_to_message.caption):
        src = u.message.reply_to_message.text or u.message.reply_to_message.caption or ""
        handles |= _merge_handles(src)

    if not handles:
        await u.message.reply_text("no handles provided")
        return

    p = pathlib.Path(MY_FOLLOWING_TXT)
    p.parent.mkdir(parents=True, exist_ok=True)
    existing = load_my_following() or set()
    merged   = existing | handles
    p.write_text("\n".join(sorted(merged)), encoding="utf-8")
    n = reload_following()
    await u.message.reply_text(f"[followed-by] saved {len(handles)} new, total {n}")
# --- END PATCH ---
...
application.add_handler(CommandHandler("reload_following", cmd_reload_following))
application.add_handler(CommandHandler("setfollowing",     cmd_setfollowing))
