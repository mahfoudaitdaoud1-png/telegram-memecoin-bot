...
async def healthz():
    try:
        ...
        n = 0
    return {"ok": True, "handles": n}

# Accept webhook without token and at root
@app.post("/webhook")
async def telegram_webhook_no_token(request: Request):
    data = await request.json()
    update = Update.de_json(data, application.bot)
    await application.process_update(update)
    return {"ok": True}
...
@app.post("/webhook/")
async def telegram_webhook_trailing(request: Request):
    return await telegram_webhook_no_token(request)
